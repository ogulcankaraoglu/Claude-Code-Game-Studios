...
These items still need explicit product decisions before implementation is finalized:

- Should mine positions be fixed or randomized per attempt?
- Should failed stages be free to retry?
- Can players buy extra moves?
- Are rewards auto-claimed or manually claimed?
- Do tools consume instantly on purchase or exist in inventory?
- Does each event cycle reuse the same boards?
- Should there be a tutorial tooltip on first entry?
